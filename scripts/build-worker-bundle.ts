...
async function buildWorkerBundle(): Promise<void> {
  ...
  try {
    ...
    if (sizeInMB > 10) {
      console.warn(`\n⚠️ Warning: Bundle size (${sizeInMB.toFixed(2)} MB) exceeds Workers 10MB limit!`);
    } else if (sizeInMB > 1) {
      console.log(`   - ℹ️ Bundle requires paid Workers plan (>${1}MB)`);
    }
...
function verifyGeneratedFile(filePath: string): void {
  ...
  try {
    ...
    if (!content.includes('fetch') || !content.includes('export')) {
      console.warn('⚠️ Warning: Bundle may be incomplete - missing expected Worker patterns');
    }
