...
export function diagnoseWebSocket() {
  ...
  console.log('1️ Auth Token:', token ? ' 存在' : ' 缺失')

  // 2. 检查全局 WebSocket 连接
  const ws = diagWindow.__ws_client
  console.log('2️ WebSocket Client:', ws ? ' 存在' : ' 未初始化')
  ...
  console.log('3️ Sync Service:', syncService ? ' 存在' : ' 未初始化')
  ...
  console.log('4️ Conversations Store:', conversationsStore ? ' 存在' : ' 未初始化')
