...
export default {
  ...
  systemSettings: {
    ...
    advanced: {
      title: '高级设置',
      description: '调整消息发送、缓存、会话和监控行为',
      messaging: '消息处理',
      messageQueueSize: '消息队列大小',
      messageQueueSizeHint: '触发背压前允许排队的最大消息数量',
      messageTimeout: '消息超时 (秒)',
      messageTimeoutHint: '消息处理逾时前的等待时间',
      caching: '缓存',
      cacheExpiry: '缓存过期时间 (分钟)',
      cacheExpiryHint: '系统缓存数据保持有效的时间',
      sessionExpiry: '会话过期时间 (小时)',
      sessionExpiryHint: '已认证会话可保持活动的时间',
      features: '功能',
      enableRateLimit: '启用速率限制',
      enableRateLimitHint: '限制过量请求以保护服务稳定性',
      enableLogging: '启用系统日志',
      enableLoggingHint: '记录运行事件，便于排查问题和审计',
      enableMetrics: '启用性能监控',
      enableMetricsHint: '收集运行指标，用于监控和仪表板',
      recallWindow: {
        label: '消息可撤回时间',
