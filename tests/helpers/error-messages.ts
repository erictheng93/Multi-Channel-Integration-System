...
// 輔助函數：將英文錯誤訊息轉換為中文
export const translateError = (englishMessage: string): string => {
  return ERROR_MESSAGE_MAP[englishMessage] || englishMessage
}

// 測試輔助函數：期望錯誤訊息
export const expectErrorMessage = (actual: string, expectedKey: keyof typeof ERROR_MESSAGES) => {
  const expected = ERROR_MESSAGES[expectedKey]
  if (actual !== expected) {
    throw new Error(`Expected error message "${expected}", but got "${actual}"`)
  }
}