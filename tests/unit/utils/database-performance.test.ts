...
describe('Database Utils - Performance & Stress Testing', () => {
  ...
  describe('Memory Usage Optimization', () => {
    it('should handle streaming large result sets without memory overflow', async () => {
      const batchSize = 100
      ...
      mockDb.prepare = vi.fn().mockImplementation((query: string) => {
        if (query.includes('SELECT * FROM customers')) {
          const mockCustomers = Array.from({ length: batchSize }, (_, i) => ({
            id: i + 1,
            platform: 'line',
            platform_user_id: `U${i}`,
            display_name: `Customer ${i}`,
      ...
      for (let batch = 0; batch < totalBatches; batch++) {
        const customers = await getAllCustomers(mockDb as any, batchSize)
      ...
      expect(totalProcessed).toBe(batchSize * totalBatches)
      expect(duration).toBeLessThan(2000) // Should complete within 2 seconds
    })
  ...
  describe('Query Complexity Performance', () => {
    it('should handle complex message tree queries efficiently', async () => {
      const messageCount = 500
      const conversationId = 1

      // Generate complex message tree with replies
      const mockMessages: DbMessage[] = Array.from({ length: messageCount }, (_, i) => ({
        id: `msg_${i}`,
        conversation_id: conversationId,
        sender_type: i % 3 === 0 ? 'customer' : 'agent',
        ...
        sent_at: i % 3 !== 0 ? '2024-01-01T00:00:00Z' : undefined,
        delivery_status: i % 3 === 0 ? 'pending' : 'sent',
        reply_to_message_id: i > 0 && i % 5 === 0 ? `msg_${i - 1}` : undefined,
        thread_id: i % 10 === 0 ? `thread_${Math.floor(i / 10)}` : undefined,
        session_id: `session_${Math.floor(i / 20)}`,
        session_sequence: (i % 20) + 1,
        metadata: JSON.stringify({ index: i, type: 'test' }),
        created_at: new Date(Date.now() - (messageCount - i) * 1000).toISOString()
      }))

      // Setup mock for complex query
      mockDb.prepare = vi.fn().mockImplementation((query: string) => {
        if (query.includes('SELECT * FROM messages')) {
          return {
            bind: vi.fn().mockReturnThis(),
      ...
      const repliedMessages = result.filter(m => m.reply_to_message_id)
      const threadMessages = result.filter(m => m.thread_id)
      expect(repliedMessages.length).toBeGreaterThan(0)
      expect(threadMessages.length).toBeGreaterThan(0)
    })

    it('should handle multiple concurrent complex queries', async () => {
      const queryCount = 20
      const messagesPerQuery = 100

      // Setup mock for concurrent queries
      mockDb.prepare = vi.fn().mockImplementation((query: string) => {
        if (query.includes('SELECT * FROM messages')) {
          const mockMessages = Array.from({ length: messagesPerQuery }, (_, i) => ({
            id: `msg_${i}`,
            conversation_id: 1,
            sender_type: 'customer',
            content: `Message ${i}`,
      ...
      const queries = Array.from({ length: queryCount }, (_, i) =>
        getConversationMessages(mockDb as any, i + 1, messagesPerQuery)
      )
      ...
      results.forEach((messages) => {
        expect(messages).toHaveLength(messagesPerQuery)
      })
    })
  })
  ...
  describe('Resource Cleanup and Optimization', () => {
    it('should properly clean up resources after operations', async () => {
      ...
      // Setup mock with resource tracking
      ...
      mockDb.prepare = vi.fn().mockImplementation((query: string) => {
        ...
        if (query.includes('SELECT * FROM customers WHERE platform = ? AND platform_user_id = ?')) {
          ...
          return {
            ...
            first: vi.fn().mockImplementation(() => {
              // Simulate cleanup
              setTimeout(() => activeConnections--, 10)
      ...
      expect(activeConnections).toBeLessThanOrEqual(operationCount * 0.1) // Allow some variance
    })
    ...
    it('should handle rapid sequential operations without degradation', async () => {
      ...
      // Only check if first half average is greater than 0 to avoid division issues
      if (firstHalfAvg > 0) {
        expect(secondHalfAvg).toBeLessThan(firstHalfAvg * 5) // Allow more variance for mock operations
      }
