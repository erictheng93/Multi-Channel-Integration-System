...
try {
    npx wrangler d1 execute multi-channel-platform --remote --command="ALTER TABLE customers ADD COLUMN profile_updated_at TEXT;"
} catch {
...
npx wrangler d1 execute multi-channel-platform --remote --command="PRAGMA table_info(conversations);"
...
npx wrangler d1 execute multi-channel-platform --remote --command="CREATE TABLE IF NOT EXISTS conversations (
    id TEXT PRIMARY KEY,
...
npx wrangler d1 execute multi-channel-platform --remote --command="CREATE INDEX IF NOT EXISTS idx_conversations_customer ON conversations(customer_id);"
npx wrangler d1 execute multi-channel-platform --remote --command="CREATE INDEX IF NOT EXISTS idx_conversations_status ON conversations(status);"
npx wrangler d1 execute multi-channel-platform --remote --command="CREATE INDEX IF NOT EXISTS idx_conversations_assigned_user ON conversations(assigned_user_id);"
npx wrangler d1 execute multi-channel-platform --remote --command="CREATE INDEX IF NOT EXISTS idx_conversations_assigned_team ON conversations(assigned_team_id);"

# 5. Verify tables exist
Write-Host "✅ Verifying table structure..."
npx wrangler d1 execute multi-channel-platform --remote --command="SELECT name FROM sqlite_master WHERE type='table';"
