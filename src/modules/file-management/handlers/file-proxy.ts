...
fileProxyHandler.get('/public/*', async (c) => {
  try {
    ...
    // F7: require a valid HMAC signature on the URL. Previously this
    // endpoint was wide-open and let anyone with a guessed r2Key download
    // arbitrary R2 content. The signature is generated by signFileUrl()
    // when an attachment URL is minted, so legitimate LINE / agent links
    // continue to work; bare key guesses fail with 404 (404 not 401 to
    // avoid leaking the existence of any particular r2Key).
    const sig = c.req.query('sig');
    const exp = c.req.query('exp');
    const sigValid = await verifyFileSignature(r2Key, sig, exp, c.env.JWT_SECRET);
    if (!sigValid) {
      log.warn('File proxy: missing or invalid signature', { r2Key, hasSig: !!sig });
...
fileProxyHandler.get('/download/:attachmentId', async (c) => {
  try {
    ...
    // F7: signature must match the resolved r2Key. Signing against r2Key
    // (not attachmentId) means a stolen download URL leaks at most one
    // file, not any file the attacker can correlate to an attachment id
    // they discover later.
    const sig = c.req.query('sig');
    const exp = c.req.query('exp');
    const sigValid = await verifyFileSignature(r2Key, sig, exp, c.env.JWT_SECRET);
    if (!sigValid) {
      log.warn('Download: missing or invalid signature', { attachmentId, hasSig: !!sig });
      return c.json({ success: false, error: 'Attachment not found' }, HTTP_STATUS.NOT_FOUND);
    }
