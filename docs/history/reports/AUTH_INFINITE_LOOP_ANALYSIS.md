...
### 2️ **次要問題：Auth Store 初始化時的 Token 有效性檢查**
  ...
  if (isSessionValid && storedToken) {
    ...
    if (storedAgent) {
      try {
        ...
        clearAuthStorage();  // 解析失敗時清除
      }
    }
  } else {
    clearAuthStorage();  // session 過期時清除
  }
}
```

**潛在問題：**
- 只檢查 `sessionExpiry`（前端設置的 7 天過期時間）
- 沒有檢查 JWT token 本身的 `exp` 字段（後端設置的過期時間）
- 如果 JWT token 已過期但 sessionExpiry 還沒過期，會導致：
...
### 3️ **衝突問題：Login.vue 的 watch 與路由守衛衝突**
...
export async function combinedAuthGuard(
  ...
  console.log(' Auth Guard:', to.path)
