...
if (conflicts.length > 0) {
  console.warn('\n⚠️  WARNING: Route conflicts detected!');
...
// 可選: 檢查衝突
if (conflicts.length > 0) {
  console.warn('⚠️  Route conflicts detected in Teams module!');
}
```
...
if (process.env.NODE_ENV === 'development' && conflicts.length > 0) {
  console.warn(`⚠️  [${import.meta.url}] Route conflicts detected!`);
}
