...
private async loadState(): Promise<void> {
  try {
    ...
    console.error('⚠️ [LatestMessageCacheCoordinator] Failed to load state:', error);
...
private async saveState(): Promise<void> {
  try {
    ...
    console.error('⚠️ [LatestMessageCacheCoordinator] Failed to save state:', error);
...
修复前:
┌─────────────────────────────────────────────────────────────┐
│ ⚠️  1 Unhandled Rejection 错误                              │
│ ⚠️  31 个集成测试因配置错误无法运行                          │
│ ⚠️  Mock 设置无标准，开发者各自为政                          │
│ ⚠️  测试基础设施不稳定                                       │
└─────────────────────────────────────────────────────────────┘
